...
use NeoSimbio\Utils\SimbioSecurity;
        ...
        if (isset($_SERVER['QUERY_STRING']) AND !empty($_SERVER['QUERY_STRING'])) {
            ...
            foreach ($arr_query_var as $varname => $varvalue) {
                if (is_string($varvalue)) {
                    ...
                    if ($varname != 'page') {
                        $_query_str_page .= SimbioSecurity::xssFree($varname).'='.SimbioSecurity::xssFree($varvalue).'&';
                    }
                } else if (is_array($varvalue)) {
                    foreach ($varvalue as $e_val) {
                        if ($varname != 'page') {
                            $_query_str_page .= SimbioSecurity::xssFree($varname).'[]='.SimbioSecurity::xssFree($e_val).'&';
                        }
                    }
                }
            }
