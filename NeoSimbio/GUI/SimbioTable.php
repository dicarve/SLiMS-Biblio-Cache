<?php
/**
 * \Simbio\GUI\SimbioTable class
 * Class for creating HTML table
 *
 * Copyright (C) 2023  Arie Nugraha (dicarve@gmail.com)
 *
...
class SimbioTableRow
{
    ...
    /**
     * Method to create SimbioTableField array from array
     *
    ...
    public function addFields($array_field_content)
    {
        foreach ($array_field_content as $idx => $fld_content) {
            $_field_obj = new SimbioTableField();
...
class SimbioTable
{
    ...
    public function setHeader($array_column_content)
    {
        if (!is_array($array_column_content)) {
            ...
            $this->table_row[0] = new SimbioTableRow($array_column_content);
        }
    }
    ...
    public function appendTableRow($array_column_content)
    {
        ...
        if (!is_array($array_column_content)) {
            ...
            // create instance of SimbioTableRow
            $_row_obj = new SimbioTableRow($array_column_content);
            if ($_row_cnt < 1) {
    ...
    public function printTable()
    {
        ...
        if (count($this->table_row) < 1) {
            ...
            foreach ($this->table_row as $_row_idx => $_row) {
                if (!$_row instanceof SimbioTableRow) {
                    continue;
                }
